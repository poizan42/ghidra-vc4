...
public class SlaFormat {
	...
	/**
	 * Holds the body of every `outlined` macro, in macro-index order, so a call site can carry a
	 * reference instead of a copy of the body. A call is an ordinary op_tpl with opcode CAST whose
	 * first input is the index into this table -- the same representation the sleigh compiler
